...
- **Auto-fixes** all mechanical issues immediately (no reasoning required):
  ...
  - Source Metadata/ folder contains a depositor manifest file (name matches `sample`, `metadata`, `manifest`, `clinical`, `samplesheet`, `sample_list`, `sample_info` with `.xlsx`/`.csv`/`.tsv`/`.xls` extension) AND any File entity has placeholder values (`Not Applicable`, `Unknown`, `NA`) for biological per-sample fields (diagnosis, tumorType, sex, age, genotype, tissue) → **BLOCKING ERROR**: mark project with `unparsed_depositor_manifest`; Phase 2 must download the manifest, map rows to files, and apply per-sample ground-truth values before the block can clear. See "depositor-manifest-must-be-parsed-not-just-stored" pattern.
  - Any File entity has `specimenType='cell line'` or `isCellLine='Yes'` and the bound schema includes an `isPrimaryCell` property and the file has no `isPrimaryCell` annotation → **AUTO-FIX**: set `isPrimaryCell='No'` (immortalized cell lines are by definition not primary cells). See "isprimarycell-no-for-cell-lines" pattern.
  - A single repository accession produced more than one Dataset entity AND the file folders contain a mix of imaging (.tif/.czi/.ome.tif/.dcm/.svs) and sequencing (.fastq/.bam/.h5/.h5ad/.mtx/count matrix) files → **BLOCKING ERROR**: mark project with `spatial_transcriptomics_split_error`; merge into a single Dataset entity covering all files (keep separate schema-bound folders per modality, but the Dataset entity must be one). See Standard 15 and "spatial-transcriptomics-one-dataset-confirmed" pattern.
